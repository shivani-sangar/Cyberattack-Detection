...
class DBN(object):
	...
	def pretraining(self, sess, train_set_x, batch_size = 100, pretraining_epochs = 1,
		learning_rate = 0.0001, k = 1, display_step = 1): #pretraining_epochs = 10
...
#I have just changed some lines of code in this function to evaluate correctly


	def fine_tuning(self, sess, train_set_x, batch_size = 10 , training_epochs = 1 , learning_rate = 0.1, display_step = 1):
		...
		for epoch in range(training_epochs):
			...
			if epoch % display_step == 0:
				...
				with open("kdd_output.csv", "a", newline='') as csv_file:
					csvapp = csv.writer(csv_file)
					csvapp.writerow([epoch + 1, ACC_max, avg_cost])
...
if __name__ == "__main__":
	...
	filename1 = dir_path + "/datasets/kddcup.data_10_percent_corrected"
	#filename1 = dir_path + "/datasets/kddcup.data.corrected"
	filename2 = dir_path + "/datasets/corrected"
	...
	scaler = MinMaxScaler()
	dataset1[num_features] = scaler.fit_transform(dataset1[num_features])
#	dataset1[num_features] = dataset1[num_features].apply(lambda x:MinMaxScaler().fit_transform(x))
	...
	scaler = MinMaxScaler()
	dataset2[num_features] = scaler.fit_transform(dataset2[num_features])
...
#	dataset2[num_features] = dataset2[num_features].apply(lambda x:MinMaxScaler().fit_transform(x))
	...
	with tf.Session() as sess:
		...
		dbn.fine_tuning(sess, train_set_x = acc, training_epochs = training_epochs)
