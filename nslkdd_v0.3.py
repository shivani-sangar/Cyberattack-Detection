...
class DBN(object):
	...
	def fine_tuning(self, sess, train_set_x, batch_size=100, training_epochs=200, learning_rate=0.1, display_step=1):
		...
		start_time = timeit.default_timer()
		train_ops = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(self.finetune_cost,
		...
		for epoch in range(training_epochs):
			...
			if epoch % display_step == 0:
				...
				end_time = timeit.default_timer()
				print("Time {0} minutes".format((end_time - start_time)))
				...
				d = np.append(d, sess.run(tf.argmax(self.y, axis=1), feed_dict={self.x: train_set_x.test.segments,
				...
				print("Epoch:", epoch + 1, " Accuracy:", ACC_max, " Cost:", avg_cost)

				with open("nslkdd_output.csv","a",newline='') as csv_file:
...
if __name__ == "__main__":
	...
	filename1 = dir_path + "/datasets/NSL_KDD/KDDTrain.csv"
	filename2 = dir_path + "/datasets/NSL_KDD/KDDTest.csv"
